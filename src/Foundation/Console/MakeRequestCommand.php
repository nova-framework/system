...
class MakeRequestCommand extends Command
{
    ...
    public function fire()
    {
        ...
        $stub = $this->files->get(__DIR__ .DS .'stubs' .DS .'request.stub');
    ...
    protected function addNamespace($stub)
    {
        if (! is_null($namespace = $this->input->getOption('namespace'))) {
            return str_replace('{{namespace}}', 'namespace App\Requests\\' .$namespace .';', $stub);
        } else {
            return str_replace('{{namespace}}', 'namespace App\Requests;', $stub);
        }
    }
    ...
    protected function getPath()
    {
        ...
        if (is_null($path)) {
            return $this->nova['path'] .DS .'Requests';
        } else {
    ...
    {
        return array(
            array('name', InputArgument::REQUIRED, 'The name of the Request.'),
        );
    }
    ...
    {
        return array(
            array('path', null, InputOption::VALUE_OPTIONAL, 'The path where the Request should be stored.', null),
            array('namespace', null, InputOption::VALUE_OPTIONAL, 'The Request namespace.', null),
        );
    }
}
