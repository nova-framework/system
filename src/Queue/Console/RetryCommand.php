<?php namespace Nova\Queue\Console;
...
class RetryCommand extends Command
{
    ...
    {
        $failed = $this->nova['queue.failer']->find($this->argument('id'));

        if ( ! is_null($failed)) {
            $failed->payload = $this->resetAttempts($failed->payload);

            $this->nova['queue']->connection($failed->connection)->pushRaw($failed->payload, $failed->queue);

            $this->nova['queue.failer']->forget($failed->id);
